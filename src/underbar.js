(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(val, index) {
      outputArr.push(iterator(val, index));
    });
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var acc = accumulator
    if (acc === undefined){
      //how to start when no acc given using element 1
      acc = collection[0];
  ...
  _.every = function(collection, iterator) {
    ...
    //type coercion very important here and tricky
  ...
  _.memoize = function(func) {
    ...
    //array(?) for argument list passed
    //array(?) for results of that list (on the same index)
    var result;
    var resultsObj = {}
  ...
  _.delay = function(func, wait) {
    var argArr = Array.from(arguments).slice(2);
  ...
  _.shuffle = function(array) {
    ...
    while (copy.length > 0) {
      var randIndex = getRandomInt(copy.length);
